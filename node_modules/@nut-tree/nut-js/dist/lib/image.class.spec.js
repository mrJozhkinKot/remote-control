...
describe("Image class", () => {
    it("should return alphachannel = true for > 3 channels", () => {
        const SUT = new image_class_1.Image(200, 200, Buffer.from([123]), 4, "id");
        expect(SUT.hasAlphaChannel).toBeTruthy();
    });
    it("should return alphachannel = false for <= 3 channels", () => {
        const SUT = new image_class_1.Image(200, 200, Buffer.from([123]), 3, "id");
        expect(SUT.hasAlphaChannel).toBeFalsy();
    });
    it("should return alphachannel = false for <= 3 channels", () => {
        const SUT = new image_class_1.Image(200, 200, Buffer.from([123]), 2, "id");
        expect(SUT.hasAlphaChannel).toBeFalsy();
    });
    it("should return alphachannel = false for <= 3 channels", () => {
        const SUT = new image_class_1.Image(200, 200, Buffer.from([123]), 1, "id");
        expect(SUT.hasAlphaChannel).toBeFalsy();
    });
    it("should throw for <= 0 channels", () => {
        expect(() => new image_class_1.Image(200, 200, Buffer.from([123]), 0, "id")).toThrowError("Channel <= 0");
    });
    ...
    describe("Colormode", () => {
        it("should not try to convert an image to BGR if it already has the correct color mode", async () => {
            // GIVEN
            const bgrImage = new image_class_1.Image(100, 100, Buffer.from([]), 3, "testImage");
            // WHEN
        ...
        it("should not try to convert an image to RGB if it already has the correct color mode", async () => {
            // GIVEN
            const rgbImage = new image_class_1.Image(100, 100, Buffer.from([]), 3, "testImage", colormode_enum_1.ColorMode.RGB);
            // WHEN
    ...
    describe('isImage typeguard', () => {
        it('should identify an Image', () => {
            // GIVEN
            const img = new image_class_1.Image(100, 100, Buffer.from([]), 4, 'foo');
            // WHEN
