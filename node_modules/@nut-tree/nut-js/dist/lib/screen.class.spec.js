...
const providerRegistryMock = (0, sneer_1.mockPartial)({
    getScreen() {
        return (0, sneer_1.mockPartial)({
            grabScreenRegion() {
                return Promise.resolve(new image_class_1.Image(searchRegion.width, searchRegion.height, Buffer.from([]), 3, "needle_image"));
            },
            screenSize() {
...
describe("Screen.", () => {
    describe("find", () => {
        it("should resolve with sufficient confidence.", async () => {
            ...
            const needle = new image_class_1.Image(100, 100, Buffer.from([]), 3, "needle_image");
            ...
            const matchRequest = new match_request_class_1.MatchRequest(expect.any(image_class_1.Image), needle, SUT.config.confidence, true);
        ...
        it("should reject with insufficient confidence.", async () => {
            ...
            const needle = new image_class_1.Image(100, 100, Buffer.from([]), 3, id);
            // WHEN
        ...
        it("should override default confidence value with parameter.", async () => {
            ...
            // THEN
            await expect(resultRegion).resolves.toEqual(matchResult.location);
            const matchRequest = new match_request_class_1.MatchRequest(expect.any(image_class_1.Image), needle, minMatch, true);
            expect(findMatchMock).toHaveBeenCalledWith(matchRequest);
        });
        it("should override default search region with parameter.", async () => {
            ...
            const expectedMatchRequest = new match_request_class_1.MatchRequest(expect.any(image_class_1.Image), needle, SUT.config.confidence, true);
            // WHEN
        ...
        it("should override searchMultipleScales with parameter.", async () => {
            ...
            const SUT = new screen_class_1.ScreenClass(providerRegistryMock);
            const needle = new image_class_1.Image(100, 100, Buffer.from([]), 3, "needle_image");
            const parameters = new optionalsearchparameters_class_1.OptionalSearchParameters(searchRegion, undefined, false);
            const expectedMatchRequest = new match_request_class_1.MatchRequest(expect.any(image_class_1.Image), needle, SUT.config.confidence, false);
            // WHEN
            await SUT.find(needle, parameters);
            // THEN
            expect(findMatchMock).toHaveBeenCalledWith(expectedMatchRequest);
        ...
        it("should override both confidence and search region with parameter.", async () => {
            ...
            const expectedMatchRequest = new match_request_class_1.MatchRequest(expect.any(image_class_1.Image), needle, minMatch, true);
            // WHEN
            await SUT.find(needle, parameters);
            // THEN
            expect(findMatchMock).toHaveBeenCalledWith(expectedMatchRequest);
        ...
        it("should add search region offset to result image location", async () => {
            ...
            const SUT = new screen_class_1.ScreenClass(providerRegistryMock);
            // WHEN
            const matchRegion = await SUT.find(new image_class_1.Image(100, 100, Buffer.from([]), 3, "needle_image"), {
    ...
    describe("findAll", () => {
        ...
        it("should add search region offset to result image location", async () => {
            ...
            const SUT = new screen_class_1.ScreenClass(providerRegistryMock);
            // WHEN
            const [matchRegion] = await SUT.findAll(new image_class_1.Image(100, 100, Buffer.from([]), 3, "needle_image"), {
                searchRegion: limitedSearchRegion
            });
            // THEN
            expect(matchRegion).toEqual(expectedMatchRegion);
    ...
    describe("capture", () => {
        it("should capture the whole screen and save image", async () => {
            // GIVEN
            const screenshot = new image_class_1.Image(100, 100, Buffer.from([]), 4, "test");
            ...
            const expectedData = { image: screenshot, path: expectedImagePath };
            // WHEN
    ...
    describe("captureRegion", () => {
        it("should capture the specified region of the screen and save image", async () => {
            // GIVEN
            const screenshot = new image_class_1.Image(100, 100, Buffer.from([]), 4, "test");
            const regionToCapture = (0, sneer_1.mockPartial)({ top: 42, left: 9, height: 10, width: 3.14159265359 });
            const grabScreenMock = jest.fn(() => Promise.resolve(screenshot));
            const saveImageMock = jest.fn();
            providerRegistryMock.getScreen = jest.fn(() => (0, sneer_1.mockPartial)({
